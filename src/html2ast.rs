...
pub fn lexer(
    ...
    let comment_end_string = String::from("-->");
    let exclamation_string = String::from("!");
    let bar_string = String::from("-");
    let comment_string = String::from("comment");
    let double_quotes = String::from('"');
    ...
    if *item == left_angle_bracket {
        ...
        } else if file.get(index..index + 1).ok_or("comment ! err")?[0] == exclamation_string {
            index += 1;
            let mut cur = file
                .get(index..index + 1)
